...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($applicant_id) && isset($_POST['action']) && $_POST['action'] === 'register') {
        ...
        if (!is_password_valid_strong($passwordRaw)) $validationErrors[] = 'Password must be at least 8 characters and contain at least one uppercase letter, one number, and one special character.';
        ...
        if (count($validationErrors) > 0) {
            ...
            // uniqueness check - check each field individually for specific error messages
            $duplicateErrors = [];
            
            // Check PF Number
            $stmt = $conn->prepare("SELECT applicant_id FROM applicants WHERE pf_no = ? LIMIT 1");
            $stmt->bind_param("s", $pf_number);
            $stmt->execute();
            if ($stmt->get_result()->num_rows > 0) {
                $duplicateErrors[] = "PF Number already exists.";
            }
            
            // Check Username
            $stmt = $conn->prepare("SELECT applicant_id FROM applicants WHERE username = ? LIMIT 1");
...
<head>
    ...
    <style>
        ...
        .password-feedback .constraint {
            margin: 6px 0;
            display: flex;
            align-items: center;
            gap: 8px;
        }
        .password-feedback .constraint .icon {
        ...
        .password-feedback .constraint.valid {
            color: #155724;
        }
        .password-feedback .constraint.valid .icon {
            color: #28a745;
        }
    </style>
</head>
...
<div class="container">
    ...
    <div class="right-panel">
        <div class="card">
            <div style="margin-bottom:12px;"><button onclick="history.back();" class="btn btn-secondary">Back</button></div>
            ...
            <?php if (!$applicant_id): ?>
                ...
                <?php if ($just_registered): ?>
                    <div style="text-align:center;margin-top:6px;">
                        ...
                        <p style="margin-top:12px;color:#666;">A verification email has been sent to your email address. Please check your email and click the verification link to complete your registration</p>
                        <div style="margin-top:12px;">
                            <a href="applicantlogin.php" class="btn btn-primary">Back to Login</a>
                        </div>
                    </div>
                <?php else: ?>
                <form method="POST" action="" enctype="multipart/form-data" class="compact-form">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="pf_number" required value="<?= htmlspecialchars($applicant['pf_number'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label>Username</label>
                        <input type="text" name="username" required value="<?= htmlspecialchars($applicant['username'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label>Password</label>
                        <input type="password" id="reg_password" name="password" required value="">
                        <div id="passwordFeedback" class="password-feedback" style="display:none;margin-top:8px;">
                            <div class="constraint" id="constraint_length"><span class="icon">✗</span> At least 8 characters</div>
                            <div class="constraint" id="constraint_uppercase"><span class="icon">✗</span> At least one uppercase letter (A-Z)</div>
                            <div class="constraint" id="constraint_number"><span class="icon">✗</span> At least one number (0-9)</div>
                            <div class="constraint" id="constraint_special"><span class="icon">✗</span> At least one special character (!@#$%^&* etc)</div>
                        </div>
                    </div>
                    <div class="form-group">
                        <label>Confirm Password</label>
                        <input type="password" id="reg_password_confirm" name="password_confirm" required value="">
                    </div>
                    <div class="form-group">
                        <label>Full name</label>
                        <input type="text" name="name" required value="<?= htmlspecialchars($applicant['name'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label>Email</label>
                        <input type="email" name="email" required value="<?= htmlspecialchars($applicant['email'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label>Contact</label>
                        <input type="text" name="contact" required value="<?= htmlspecialchars($applicant['contact'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label>Next of Kin Name</label>
                        <input type="text" name="next_of_kin_name" required value="<?= htmlspecialchars($applicant['next_of_kin_name'] ?? '') ?>">
                    </div>
                    <div class="form-group">
                        <label>Next of Kin Contact</label>
                        <input type="text" name="next_of_kin_contact" required value="<?= htmlspecialchars($applicant['next_of_kin_contact'] ?? '') ?>">
                    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateFeedback() {
        ...
        // Show feedback only when user starts typing
        ...
        // Update each constraint indicator
        const constraintIds = ['length', 'uppercase', 'number', 'special'];
        constraintIds.forEach(id => {
            const element = document.getElementById('constraint_' + id);
            const isValid = constraints[id];
    ...
    passwordInput.addEventListener('focus', updateFeedback);
});
</script>
<script>
document.addEventListener('DOMContentLoaded', function(){
