...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['apply_house'])) {
    ...
    // Enforce one application per category per applicant (ignore cancelled and forfeited applications)
    // Applicants can reapply after forfeiting an application
    $chk = $conn->prepare("SELECT COUNT(*) AS cnt FROM applications WHERE applicant_id = ? AND category = ? AND LOWER(status) NOT IN ('cancelled', 'forfeit')");
...
<div class="main-content">
    ...
    <?php if (!empty($apply_error)): ?>
        <div style="background:#e3f2fd;padding:12px;border:1px solid #bbdefb;border-radius:6px;margin-bottom:12px;color:#1a237e;font-weight:700;">
    ...
    <table>
        ...
        ?>
            <tr id="row-<?= htmlspecialchars($row['application_id']) ?>" data-app-id="<?= htmlspecialchars($row['application_id']) ?>">
                ...
                <td>
                    <?php if (strtolower($normStatus) !== 'cancelled' && strtolower($normStatus) !== 'won' && strtolower($normStatus) !== 'allocated'): ?>
                        <a href="forfeit_form.php?app_id=<?= htmlspecialchars($row['application_id']) ?>" style="color:#ff9800;text-decoration:none;font-weight:700;" title="Forfeit this application">Forfeit</a>
                    <?php else: ?>
                        -
                    <?php endif; ?>
                </td>
