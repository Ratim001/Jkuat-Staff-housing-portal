...
<head>
    ...
    .terminate-btn { background-color: #ff9800; }
        ...
        <tbody>
            ...
            ?>
                <tr>
                    ...
                    <td>
                        <button class="btn btn-info btn-sm" onclick="openNotifyForm('<?= $row['tenant_id'] ?>')">Notify</button>
                        <button class="btn btn-danger btn-sm" onclick="terminateTenant('<?= $row['tenant_id'] ?>')" <?= ($row['status'] == 'Terminated') ? 'disabled' : '' ?>>Terminate</button>
                    </td>
                </tr>
            <?php } ?>
        </tbody>
